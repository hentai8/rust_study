...
fn main() {
    ...
    let now_seconds = now.format("%Y%m%d%H%M%S");
...
fn get_utc_time(timestamp: i64, format: &str) -> String {
    let datetime = DateTime::<Utc>::from_utc(NaiveDateTime::from_timestamp(timestamp / 1000, 0), Utc);
